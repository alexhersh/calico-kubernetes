.PHONY: all binary binaries policy_agent ut clean run-service-proxy run-kubernetes-master

SRCFILES=$(shell find calico_kubernetes)
POLICY_SRCFILES=$(shell find policy_agent)
...
policy_agent: $(POLICY_SRCFILES)
	...
	calico/build pyinstaller policy_agent/policy_agent.py -a -F -s --clean
